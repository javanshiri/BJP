...
# calculate f(y|x)
#####################
path <- "E:\\Project-proposal\\projects\\Lake Urmia\\output\\"
setwd (path)
...
for (v in 1:length.sample)
{ 
  P <- read.csv(mcmc.sample[v]) [,-1]
  data_test <- read.csv (paste0(path, file.names[m],"\\data_test\\", mcmc.sample[v]))[,-1]
  landa <- read.csv (paste0(path, file.names[m],"\\landa\\", mcmc.sample[v]))[,-1]
  z1_model <- data_test[,4]
  mu_perim <- lapply(z1_model, F= function (x) {P[,"mu2"] + P[,"roh"] * P[,"sigma2"] / P[,"sigma1"] * (x - P[,"mu1"])}) 
  sigma_perim <- P[,"sigma2"] ^ 2 * (1 - P[,"roh"] ^ 2)
  ...
  write.csv(prob_obs, paste0( path, file.names[m], "\\prob\\", v, ".csv"))
}
 }
#####################################################  
